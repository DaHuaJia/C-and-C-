...
struct student_info  {  
	...
	char adress[20]; //家庭住址  
	char tel[10];    //电话   
	int chinese,math,english,huping,pingde,jiaoping,paiming1,paiming2;   //语文 数学  英语  互评  品德  教评      
...
void xiugai(); 
void DeleteInfo();  
void paiming2(struct student_info student[N]); 
void ViewAll(); 
void clean(); 
void PaimingView(); 
void exits();  
...
void main(){  
	...
	switch(select){    
		...
		case '5': system("cls");paiming1(student);paiming2(student);PM=1;getchar();getchar();system("cls"); break; //选项5:学生信息排名     
